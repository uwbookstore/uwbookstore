...
if (typeof Storage !== 'undefined') {
  // PAUSE/PLAY CONTROLS FOR FOOTER MARQUEE
  const marqueeControl = document.getElementById('marquee-play-pause');
  const marqueeTrack = document.querySelector('.marquee-content-track');

  function playMarqueeAnimation() {
    marqueeTrack.style.animationPlayState = 'running';
    marqueeControl.setAttribute('aria-pressed', 'false');
    marqueeControl.innerHTML = `
  ...
  function stopMarqueeAnimation() {
    ...
    marqueeControl.setAttribute('aria-pressed', 'true');
    marqueeControl.innerHTML = `
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-6">
              <path fill-rule="evenodd"
  ...
  if (!localStorage.getItem('playPause')) {
    marqueeTrack.style.animationPlayState = 'running';
    marqueeControl.setAttribute('aria-pressed', 'false');
    marqueeControl.setAttribute('aria-label', 'Pause scrolling content');
    marqueeControl.innerHTML = `
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-6"
              aria-hidden="true">
              <path fill-rule="evenodd"
    ...
    if (localStorage.getItem('playPause') === 'running') {
  ...
  function toggleMarquee() {
    if (marqueeControl.getAttribute('aria-pressed') === 'false') {
      stopMarqueeAnimation();
      localStorage.setItem('playPause', 'paused');
    } else {
      playMarqueeAnimation();
      localStorage.setItem('playPause', 'running');
    }
  }

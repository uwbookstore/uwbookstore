$(document).ready(function () {
  ...
  $('<p class="center red">Gift Card Verification Checkbox</p>').prependTo('#campusVerify');
  ...
  $('select#selectedMethod').change(function () {
    ...
    switch (shipType) {
      ...
      case '1278':
        $('#shipdesc').show();
        $('#msg-text').html('<div class="alert alert-danger">This shipping method is to be used for International Orders only. If you choose this method and are shipping domestically, the shipping method will be corrected to "Ground Shipping" and you will be charged accordingly.</div>').show();
        break;
      case '2193':
        $('#shipdesc').show();
        $('#msg-text').html('<div class="alert alert-danger">This shipping method is to be used for orders over $75 (pre-tax subtotal). If you choose this method and your order total is less than $75, the shipping method will be corrected to "Ground Shipping" and you will be charged accordingly.</div>').show();
        break;
      case '2298':
        $('#shipdesc').show();
        $('#msg-text').html('<div class="alert alert-danger">*$1.28 flat rate ground shipping to a contiguous U.S. delivery address. Excludes Lands\' End items, textbooks, all StandardChair of Gardner products, custom orders, tech items, and drop-ship items. <strong>Orders with any excluded items will be charged the full standard shipping fee.</strong> All items with a handling charge will receive $1.28 ground shipping but handling charge will still be applied.</div>').show();
        break;
      default:
        $('#msg-text').hide();
    }
  ...
  $("select#selectedMethod option:contains('GC Only Ground est. $3.50')").text('Gift Card Only Ground est. $3.50');
  $("select#selectedMethod option:contains('GC 2nd Day est. $20.00')").text('Gift Card 2nd Day est. $20.00');
  $("select#selectedMethod option:contains('GC Next Day est. $30.00')").text('Gift Card Next Day est. $30.00');
  ...
  switch (pickUpLocation) {
    case '970':
    case '1626':
      $('<div id="shipDisclaimer" class="alert alert-danger">Please note: Pickup at Brookfield location can take up to 7 days.  Please click on the Shipping Policy above for details.</div>').insertAfter('select#selectedMethod');
      break;
    case '1957':
      $('<div id="shipDisclaimer" class="alert alert-danger"><strong>CONTACTLESS PICK-UP AVAILABLE</strong><br>Park in front of sandwich board in front of store.<br>Call 310-5976 when you arrive.</div>').insertAfter('select#selectedMethod');
      break;
    case '968':
      $('<div id="shipDisclaimer" class="alert alert-danger"><strong>CONTACTLESS PICK-UP AVAILABLE</strong><br>Park in valid parking spots directly across from the store<br>Call 310-5980 when you arrive.</div>').insertAfter('select#selectedMethod');
      break;
    default:
      break;
  }

$(document).ready(function () {
  ...
  $('.retPolicy').html(
    ...
    $(".panel.panel-info.Dynamic_Items").addClass("container").appendTo(".featured__items-block");
    ...
    if (baseUrl === "https://www.uwbookstore.com/") {
        if ($('#h_nav a:contains("Log Out")').length) {
            ...
            $("#login").html(
                ['<a href="' + baseUrl + 'login"><span>login</span> <em class="fa fa-user"></em></a>'].join("\n")
            );
        }
    } else if (baseUrl === "https://text.uwbookstore.com/") {
        if ($('#h_nav a:contains("Log Out")').length) {
    ...
    $('<div class="alert alert-default"><p>Don\'t have a University Book Store account?</p></div>').appendTo(
        "#loginPanel"
    );
    $("a.registerHereLink").removeClass("bottom10").addClass("btn btn-text").appendTo(".alert.alert-default");
    $("a.registerHereLink").attr(
        "href",
        "https://secure2.mbsbooks.com//CustomerRegister?s=" + subUrl + "&ReturnUrl=customeraccount"
    );

    $("p.registerHere, p.forgotPassword").hide();

    // Lost Password
    $('<h1 id="lost-heading" class="heading__line-center"><span>Lost Password</span></h1>').prependTo(
        "#lostPasswordPanel"
    );
    $(
    ...
    if (baseUrl === "https://www.uwbookstore.com/") {
        $("div#list-container").html(
            [
                ...
                '<a href="' + baseUrl + 'Customer-Help#faq" class="list-group-item textc">View our FAQ</a>',
                '<a href="' + baseUrl + 'Customer-Help#return" class="list-group-item textc">Return an order</a>',
                "</div>",
            ].join("\n")
        );
        ...
        $("div#list-container").html(
            [
                '<h2 class="textc">Customer Help</h2>',
                '<div class="list-group">',
                '<a href="' + baseUrl + 'SiteText?id=2369#faq" class="list-group-item textc">View our FAQ</a>',
                '<a href="' + baseUrl + 'SiteText?id=2369#return" class="list-group-item textc">Return an order</a>',
                "</div>",
            ].join("\n")
        );
    ...
    if (
        ...
        $('<a href="' + baseUrl + 'home" class="btn btn-primary">Continue Shopping</a>').insertAfter(
            "a#Shopping_Button"
        );
    ...
    if (
        ...
        $("div.LocationBar").first().removeClass("LocationBar").addClass("breadCrumbs");
        ...
        // Point Summary Table
        $('<table id="loyaltyPoints">\n<thead>\n</thead>\n<tbody>\n</tbody>\n</table>').appendTo("div#content-left");
        $(
        ...
        $('<div id="redeem-link" class="group"></div>').appendTo("div#content-left");
        ...
        $('<div id="history-link" class="group"></div>').appendTo("div#content-left");

        $("a[id$=lnkViewHistory]").css({ "margin-top": "20px", display: "inline-block" }).appendTo("div#history-link");

        $("table#ctl00_ctl00_Content_Content_dgLoyalty").removeAttr("style border cellspacing class rules");

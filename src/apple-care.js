...
const merchWrapperP = document.querySelector('.merch-wrapper p');
const merchWrapperA = document.querySelector(
  '.merch-wrapper a.black.hover_pointer'
);

for (let i = 0; i < skuArr.length; i += 1) {
  if (
    merchWrapperP.textContent.indexOf(skuArr[i][0]) !== -1 &&
    merchWrapperA.textContent.toLowerCase().indexOf('applecare') === -1
  ) {
    if (!sessionStorage.getItem('apple-care')) {
      const appleCareBlock = document.createElement('div');
      appleCareBlock.id = 'apple-care';
      appleCareBlock.classList.add('modal fade');
      appleCareBlock.setAttribute('tabindex', '-1');
      appleCareBlock.setAttribute('role', 'dialog');
      appleCareBlock.setAttribute('aria-labelledby', 'apple-careModal');

      appleCareBlock.innerHTML = `
      ...
      document.body.appendChild(appleCareBlock);

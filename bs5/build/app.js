$(document).ready(function () {
  function search() {
    ...
    document.location.href = 'https://www.uwbookstore.com/merchlist?searchtype=all&txtSearch=' + searchInput;
  }
  ...
  if ($('.loginMessage').length) {
    $('.loginMessage').removeClass('red').addClass('alert alert-danger wi-50 mx-auto');
    $('<em class="fa fa-exclamation-triangle mr-2"></em>').prependTo('.loginMessage');

    if ($('.loginMessage').text() === "Please use your school's single sign-on process to login.") {
      $('.loginMessage').html('<em class="fa fa-exclamation-triangle"></em> &mdash; Please use your <a href="https://www.uwbookstore.com/login">UW Student Sign In</a>');
    }

    if ($('.validation-summary-errors').length) {
      $('.validation-summary-errors ul').addClass('ul').appendTo('.loginMessage');
  ...
  $('p#studentIDText, div#billingstudentNumberWrapper').addClass('alert alert-warning');
  ...
  $('.footer .copyright').html('&copy; Copyright ' + year + ' The University Book Store<br><a href="/Privacy" class="privacy" tabindex="0">Privacy Policy</a>');
  ...
  if ($('#h_nav a:contains("Log Out")').length) {
    ...
    $('#login').html(['<a href="https://www.uwbookstore.com/customeraccount?s=www.uwbookstore.com"><span>My Account</span> <em class="fa fa-user"></em></a> <span class="logout"> / <a href="https://www.uwbookstore.com/logout">logout</a></span>'].join('\n'));
  } else {
    // console.log("You can log in...");
    $('#login').html(['<a href="https://www.uwbookstore.com/login"><span>login</span> <em class="fa fa-user"></em></a>'].join('\n'));
  }

  // LOGIN
  // $("h1.page_header").hide();
  $('#loginPanelHeader, #lostPasswordPanelHeader').hide();
  $('<h1 class="heading__line-center">Log In</h1><!--<div class="alert alert-danger"><em>Attention</em> - Guest checkout is temporarily unavailable.</div>-->').prependTo('#loginPanel');

  $('#login_UserName, #login_Password').removeClass('max_width300');
  $('<label for="login_UserName">Email/Username</label>').insertBefore('#login_UserName');
  ...
  $('<label for="login_Password">Password</label>').insertBefore('#login_Password');

  $('<div class="grid form__actions"></div>').appendTo('#loginPanel');
  $('input.loginSubmit').removeClass('bottom10').appendTo('.grid.form__actions');

  $('<div class="alert alert-danger">If checking out as a guest, please enter an email address (for transactional emails), and click "Login" to continue.</div>').insertAfter('label.radioGuest');
  // if ($("label.radioGuest span").length) {
  //   $("input.loginSubmit").attr("value", "Continue");
  //   console.log("Guest user...");
  // }
  $('a.forgotPasswordLink').attr('class', 'forgotPasswordLink btn btn-text').appendTo('.grid.form__actions');
  $('<div class="alert alert-default"><p>Don\'t have a University Book Store account?</p></div>').appendTo('#loginPanel');
  $('a.registerHereLink').removeClass('bottom10').addClass('btn btn-text').appendTo('.alert.alert-default');
  $('a.registerHereLink').attr('href', 'https://secure2.mbsbooks.com//CustomerRegister?s=www.uwbookstore.com&ReturnUrl=customeraccount');

  $('p.registerHere, p.forgotPassword').hide();

  // Lost Password
  $('<h1 id="lost-heading" class="heading__line-center"><span>Lost Password</span></h1>').prependTo('#lostPasswordPanel');
  $('<a href="javascript:history.go(-1)" class="btn btn-text"><i class="fa fa-chevron-left"></i> Return to Previous Page</a>').insertAfter('#lost-heading');
  ...
  $("li.accountLI a:contains('View Loyalty Account Information')").parent().hide();
  // Manager Orders
  $('div#ordersPanelBody div.oneOrder:last-child').removeClass('bottomBorder bottom10');
  $('div.logoutLink').hide();

  //   $('ul.breadcrumb').removeClass('background_none');
  $('ul.breadcrumb').hide();

  $('<div id="order-info" class="row"><div id="list-container" class="col-md-4"></div><div class="col-md-8"></div></div>').insertAfter('h1#ordersHeader');
  $('div#ordersPanel').detach().appendTo('div#order-info div.col-md-8');
  $('div.orderPanel').detach().appendTo('div#order-info div.col-md-8');

  $('div#list-container').html(['<h2 class="textc">Customer Help</h2>', '<div class="list-group">', '<a href="https://www.uwbookstore.com/Customer-Help#faq" class="list-group-item textc">View our FAQ</a>', '<a href="https://www.uwbookstore.com/Customer-Help#return" class="list-group-item textc">Return an order</a>', '</div>'].join('\n'));

  if (!$('div#ordersPanelBody').has('div.oneOrder').length) {
    $('<em>You have no orders.<br>If you are looking for a textbook order, please login at <a href="https://text.uwbookstore.com/login">text.uwbookstore.com</a></em>').appendTo('div#ordersPanelBody');
  }

  if (window.location.href.search(/orderdetails/) !== -1) {
    const orderNum = $('#ordersHeader').text();
    $(`<a href="https://www.uwbookstore.com/contact/?order_id=${orderNum}" class="list-group-item textc order_q">Question about this order?</a>`).appendTo('.list-group');
  }

  if ($('#h_nav a:contains("Login")').length) {
    $('#logoutPanelHeader').hide();
    $('<h1 class="heading__line-center"><span>You are now logged out</span></h1>').appendTo('#logoutPanel');
    $('.logoutText').remove();
    $('<div class="login__btns"><a href="https://www.uwbookstore.com/login" class="btn btn-secondary">Log Back In</a><a href="https://www.uwbookstore.com/home" class="btn btn-primary">UW Book Store</a></div>').appendTo('#logoutPanel');
  }
  ...
  // CHECKOUT PAGE
  $('div#registerAccount h3#registerTitle').text('Register Your Account (optional)');
  $('<br><small class="red">Limit one promo code per order</small>').appendTo('label#promoLabel');
});